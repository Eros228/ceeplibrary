...
coverInput.addEventListener("change", async (e) => {
  ...
  try {
    ...
    await showAlert("Não foi possível carregar a imagem.", "Erro ao carregar imagem");
  }
});
...
addForm.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    await showAlert("Erro ao salvar o livro: " + (err.message || "Tente novamente."), "Erro ao salvar");
  } finally {
...
async function addCopies(bookId) {
  ...
  const input = await showPrompt(
    `Quantos exemplares deseja adicionar ao total atual (${book.total})?`,
    { title: `Adicionar exemplares — ${book.name}`, defaultValue: "1", inputLabel: "Quantidade a adicionar" }
  );
  if (input === null) return;

  const qtyToAdd = parseInt(input, 10);
  if (!Number.isInteger(qtyToAdd) || qtyToAdd <= 0) {
    await showAlert("Informe um número inteiro maior que zero.", "Valor inválido");
    return;
  }
  ...
  try {
    ...
    await showAlert("Erro ao atualizar a quantidade: " + (err.message || "Tente novamente."), "Erro ao atualizar");
  }
}
...
async function deleteBook(bookId) {
  ...
  const confirmed = await showConfirm(
    `Excluir "${book.name}"? Esta ação não pode ser desfeita.`,
    "Excluir livro"
  );
  if (confirmed) {
...
// ============================================================
// AVISOS, CONFIRMAÇÕES E ENTRADAS (substituem alert/confirm/prompt)
// ============================================================

let noticeResolver = null;
let noticeMode = "alert"; // "alert" | "confirm" | "prompt"
...
function openNotice({ title, message, mode, inputLabel, inputValue, okLabel }) {
  return new Promise((resolve) => {
    ...
    if (mode === "prompt") {
      ...
      noticeInputLabel.textContent = inputLabel || "";
      noticeInput.value = inputValue != null ? inputValue : "";
    } else {
      noticeInputField.hidden = true;
      noticeInput.value = "";
    }

    noticeCancelBtn.hidden = mode === "alert";
    noticeOkBtn.textContent = okLabel || (mode === "confirm" ? "Confirmar" : "OK");

    noticeModal.hidden = false;
    document.body.style.overflow = "hidden";

    requestAnimationFrame(() => {
      if (mode === "prompt") noticeInput.focus();
      else noticeOkBtn.focus();
    });
  });
}

function showAlert(message, title = "Aviso") {
  return openNotice({ title, message, mode: "alert" }).then(() => undefined);
}

function showConfirm(message, title = "Confirmar ação") {
  return openNotice({ title, message, mode: "confirm" });
}

function showPrompt(message, { title = "Informe um valor", defaultValue = "", inputLabel = "" } = {}) {
  return openNotice({ title, message, mode: "prompt", inputValue: defaultValue, inputLabel });
}

noticeOkBtn.addEventListener("click", () => {
  if (noticeMode === "prompt") {
    closeNotice(noticeInput.value);
  } else if (noticeMode === "confirm") {
    closeNotice(true);
  } else {
...
noticeCancelBtn.addEventListener("click", () => {
  closeNotice(noticeMode === "prompt" ? null : false);
});

document.querySelectorAll("[data-notice-close]").forEach((el) =>
  el.addEventListener("click", () => {
    closeNotice(noticeMode === "prompt" ? null : noticeMode === "confirm" ? false : undefined);
  })
);
