...
// ============================================================
//  Aparência: tema claro/escuro e tamanho da letra
//  (funciona em qualquer tela, inclusive antes de fazer login —
//  a preferência fica salva no navegador, não depende da conta)
// ============================================================
...
const THEME_KEY = "ceeps-theme";
const FONT_SCALE_KEY = "ceeps-font-scale";
const FONT_SCALES = [90, 100, 112, 125, 140]; // porcentagens do tamanho de letra
...
function currentFontScale() {
  ...
  return FONT_SCALES.includes(saved) ? saved : 100;
}
...
// Aplica o que já estava salvo (o <head> já adiantou isso visualmente,
// aqui só sincronizamos o estado "active" dos botões do painel).
...
fontIncreaseBtn.addEventListener("click", () => {
  const idx = FONT_SCALES.indexOf(currentFontScale());
  applyFontScale(FONT_SCALES[Math.min(idx + 1, FONT_SCALES.length - 1)]);
});

fontDecreaseBtn.addEventListener("click", () => {
  const idx = FONT_SCALES.indexOf(currentFontScale());
  applyFontScale(FONT_SCALES[Math.max(idx - 1, 0)]);
});

fontResetBtn.addEventListener("click", () => applyFontScale(100));

a11yToggleBtn.addEventListener("click", () => {
  const willShow = a11yMenu.hidden;
  a11yMenu.hidden = !willShow;
  a11yToggleBtn.setAttribute("aria-expanded", String(willShow));
});

document.addEventListener("click", (e) => {
  if (!a11yMenu.hidden && !e.target.closest("#a11yPanel")) {
  ...
  filtered.forEach((book) => {
    ...
    card.innerHTML = `
      ...
      <div class="card-actions">
        ...
        <button class="btn-action" data-action="edit-qty" title="Alterar a quantidade total de exemplares">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" width="16" height="16" aria-hidden="true"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg>
          Quantidade
        </button>
...
async function fetchGoogleBooksCovers(term, signal) {
  try {
    const url = `https://www.googleapis.com/books/v1/volumes?q=${encodeURIComponent(term)}&maxResults=12`;
    const res = await fetch(url, { signal });
    const data = await res.json();
    return (data.items || [])
...
async function fetchCoverSuggestions(term) {
  ...
  top.forEach((item) => {
    ...
    btn.innerHTML = `
      <img src="${item.cover}" alt="Capa sugerida: ${escapeHtml(item.title)}" loading="lazy" referrerpolicy="no-referrer" />
      <span class="cover-sug-caption">${escapeHtml(item.title)}${item.year ? ` (${item.year})` : ""}</span>
    `;

    // Se a miniatura falhar ao carregar, mostra um ícone no lugar em vez de
    // remover a sugestão — a capa real ainda pode funcionar no card do livro
    // (que já tem seu próprio fallback), então não vale esconder a opção.
    const suggestionImg = btn.querySelector("img");
    suggestionImg.addEventListener("error", () => {
      suggestionImg.outerHTML = `
        <span class="cover-sug-broken" aria-hidden="true">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" width="22" height="22"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg>
        </span>
      `;
    });
